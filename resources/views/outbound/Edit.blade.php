...
<nav class="bg-gray-800 text-white shadow px-20">
  <div class="container mx-auto px-4 py-4 flex items-center justify-between">
    ...
    <ol class="flex text-sm font-medium space-x-2 text-gray-300">
      ...
      <li class="flex items-center">
        <a href="{{ route('inbounds.list') }}" class="hover:text-white transition-colors duration-200">Inbounds</a>
        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mx-2 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
        </svg>
      </li>
...
<div class="max-w-xl mx-auto mt-10 mb-10 px-6 py-8 bg-gray-800 rounded-lg shadow-lg">
    <h1 class="text-2xl font-bold text-center text-white mb-6">Edit Inbound</h1>
    ...
    <form action="{{ route('inbounds.update', $inbound->id) }}" method="POST">
        ...
        <!-- Location Code -->
        <div class="mb-5">
            <label for="code" class="block text-sm font-medium text-gray-300 mb-1">Inbound Code</label>
            <input 
                type="text" 
                id="code" 
                name="code" 
        ...
        <div class="mb-6">
            ...
            <input 
                ...
                id="created_at" 
                name="created_at" 
                value="{{ $inbound->created_at ? $inbound->created_at->format('Y-m-d') : '' }}" 
                disabled
        ...
        <div class="text-center mt-4">
            <a href="{{ route('inbounds.list') }}" class="text-sm text-blue-400 hover:text-blue-500">
                Cancel
            </a>
        </div>
    </form>
